CREATE OR REPLACE TABLE PARKING_MONTHLY_SNAPSHOT AS /*Creates a new table and replaces the old one if there is already a table*/
...
WITH PARKINGDATA AS (
  ...
  JOIN PROD_GOLD.PARKING.FACT_PARKING           AS f /* Left Join on takes everything fact_parking and joins where there is a match in the parking attributes dimension*/
    ON d.pk_parkingattributes = f.fk_parkingattributes /* the join happens on the primary and foreign key*/
  WHERE d.CARPARKLOCATION IN ('Terminal 1 Parking Plaza','Terminal 2 Parking Plaza', 'Terminal 1 Valet', 'Terminal 2 Valet') /*only selects rows where the parking lot location is from the 4 listed*/
  AND f.ERRORCODE = '' /* only selects where rows don't have any errors*/
),

TXN_SNAPSHOT AS (
  SELECT
    SNAPSHOT_MONTH,
    SUM( CASE WHEN CARPARKLOCATION = 'Terminal 1 Parking Plaza' THEN REVENUE ELSE 0 END ) AS T1_PLAZA_REVENUE, /*Case When checks the column 'CARPARKLOCATION' and when the row is 'Terminal 1 Parking Plaza' then grab the value from the REVENUE column else it's a 0 and call this new column 'T1_PLAZA_REVENUE'
    ...
    COUNT_IF(CARPARKLOCATION = 'Terminal 1 Parking Plaza') AS T1_PLAZA_TXN_COUNT, /*COUNT_IF will count the number of rows when it's 'Terminal 1 Parking Plaza' and names it T1_PLAZA_TXN_COUNT*/
    ...
    COUNT(*) AS TOTAL_TXN_COUNT, /*Count's all rows and therefore all transactions*/

    /*Count of distinct license plates by parking lot location */
    COUNT(DISTINCT IFF(CARPARKLOCATION = 'Terminal 1 Parking Plaza', PLATENO, NULL)) AS DISTINCT_T1_PLAZA_PLATENO, /*COUNT(DISTINCT IFF(COLUMN, THEN, ELSE)) AS NAME*/
...
AVG_VEHICLE_VALUE_SNAPSHOT AS (
  SELECT
    SNAPSHOT_MONTH,
    AVG( CASE WHEN CARPARKLOCATION = 'Terminal 1 Parking Plaza' THEN CLEAN_MARKET_VALUE END ) AS T1_PLAZA_SAMPLE_AVG_VEHICLE_RETAIL_VALUE, /*AVG is similar to the sum scenario using CASE WHEN to only select CLEAN_MARKET_VALUE when the parking lot location is TERMINAL 1 PARKING PLAZA*/
...
SELECT
  ...
  AV.T2_PLAZA_SAMPLE_AVG_VEHICLE_RETAIL_VALUE
